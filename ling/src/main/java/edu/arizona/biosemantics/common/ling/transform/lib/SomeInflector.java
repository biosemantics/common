...
public class SomeInflector implements IInflector {
	 ...
	 */
	public SomeInflector(IPOSKnowledgeBase posKnowledgeBase, HashMap<String, String> singulars, HashMap<String, String> plurals) { //Hong TODO load in singular-plural mapping from those learned from perl step
	...
	public String getSingular(String word) {
		try {
			if(!word.matches(".*?[a-zA-Z].*") || word.contains(")") || word.contains("(") || word.contains("]") || word.contains("[")) //not a word, e.g 3-5
				return word;
